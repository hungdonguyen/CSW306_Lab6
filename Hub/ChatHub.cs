...
namespace CSW306_Lab6.Hubs
{
    public class ChatHub : Hub
    {
        ...
        public ChatHub(ConnectionManager connectionManager, ChatDbContext db)
        {
        ...
        public async Task SendPrivateMessage(string receiverConnectionId, string user, string message)
        {
            ...
            var receiver = _connectionManager.GetAllConnections().GetValueOrDefault(receiverConnectionId);
            var chatMsg = new ChatMessage
            {
                Sender = user,
                Message = message,
